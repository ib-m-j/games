...
def Process(situation):
    ...
    global Strategy, Expectation, iterations
    accum =  0
    num_newcards = 0
    ...
    for x in range(10):
        ...
        for y in range(10):
            if y == x:
                ...
                line = line + "{} ".format(overview[(y, x)])
        print(line)

def printFirstStrategy():
    overview = {}
    for (k,v) in Strategy.items():
        res =  hasFourMinusses(k[0])
        if res:
            overview[k[1]]=v
